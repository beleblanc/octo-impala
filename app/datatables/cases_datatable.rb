class CasesDatatable
  ...
  def data
    case_details.map do |cased|
      [
        link_to(cased.case_number ,url_helpers.case_detail_path(cased.id),:class=>"btn btn-mini",data:{status:"#{cased.get_status_highlight}"}),
        h(cased.rcci),
        ...
        "#{link_to("Edit",url_helpers.edit_case_detail_path(cased.id),:class=>"btn btn-mini")}
